...
impl Url {
    ...
    fn extract_host(&self) -> String{
        let url_paths: Vec<&str> = self.url.trim_start_matches("http://").splitn(2, "/").collect();

        if let Some(index) = url_paths[0].find(':'){
            url_paths[0][..index].to_string()
        } else {
