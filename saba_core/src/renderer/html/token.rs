...
impl HtmlTokenizer {
    pub fn new(html: String) -> Self {
        Self { state: State::Data, pos: 0, reconsume: false, latest_token: None, input: html.chars().collect(), buf: String::new() }
    }
    ...
    fn append_tag_name(&mut self, c: char) {
        ...
        if let Some(t) = self.latest_token.as_mut() {
            match t {
                HtmlToken::StartTag { ref mut tag, self_closing: _, attributes: _ } | HtmlToken::EndTag { ref mut tag } => tag.push(c),
    ...
    fn take_latest_token(&mut self) -> Option<HtmlToken> {
        // create_tagメソッドによって作られた最後のトークンを返す。latest_tokenによってリセット
        assert!(self.latest_token.is_some());
...
// 80種類定義されているうち17種類+一時的なバッファを示すTemporaryBufferを定義する
#[derive(Debug, Clone, PartialEq, Eq)]
pub enum State {
