use crate::renderer::{css::token::{self, CssTokenizer}, dom::node::ElementKind::Style};
use core::{assert_eq, iter::Peekable, panic};
...
impl CssParser {
    ...
    pub fn consume_qualified_rule(&mut self) -> Option<QualifiedRule> {
        ...
        loop {
            ...
            match token {
                CssToken::OpenCurly => {
                    // 開き括弧のとき、宣言ブロックの開始を表すので、宣言ブロックの解釈を示す
    ...
    fn consume_list_of_rules(&mut self) -> Vec<QualifiedRule> {
        ...
        loop {
            ...
            match token {
                ...
                // @import, メディアクエリを洗わず @mediaなどのルールが始まることを表す
                CssToken::AtKeyword(_keyword) => {
                    let _rule = self.consume_qualified_rule();
                    // このぶらずあでは@から始まるルールはサポートしないので無視
                }
                _ => {
    ...
    fn consume_selector(&mut self) -> Selector {
        let token = match self.t.next() {
            Some(t) => t,
            None => panic!("should have a yoken but got None"),
        };
        ...
        match token {
            ...
            CssToken::Ident(ident) => {
                // a:hoverのようなセレクタはタイプセレクタとして扱うため、もしコロンが出てきた場合は宣言ブロックの開始直前までトークンを進める
...
impl Declaration {
    pub fn new() -> Self {
        Self { property: String::new(), value: ComponentValue::Ident(String::new()) }
    }
